...
public class Utils {
    ...
    static String UseEnvironmentVariable(EnvVars env, String envName, String fieldValue, String filedName) {
        if (fieldValue == null
                || fieldValue.isEmpty()
                        && (env.get(envName) != null && !(Util.fixEmptyAndTrim(env.get(envName)) == null))) {
    ...
    static String DownloadFilesOrContinue(String paths, FilePath agentWorkspace, Launcher launcher)
        ...
        for (String singlePath : splitPathFiles) {
            if (!isHttpUrl(singlePath)) {
                ...
                pathsToFilesOnAgent
                        .append(DownloadFiles(userFilesPath, launcher, singlePath))
                        .append(',');
            }
        }
        return pathsToFilesOnAgent
                .substring(0, pathsToFilesOnAgent.length() - 1)
                .trim();
    }

    private static String DownloadFiles(FilePath userFilesPath, Launcher launcher, String url)
            throws IOException, InterruptedException {
        ...
        launcher.launch().cmds(args).pwd(userFilesPath).quiet(true).join();
        return outputPath;
    }
     ...
     */
    static void deleteAppdomeWorkspacce(TaskListener listener, FilePath appdomeWorkspace)
            throws IOException, InterruptedException {
        listener.getLogger().print("Deleting temporary files." + System.lineSeparator());
